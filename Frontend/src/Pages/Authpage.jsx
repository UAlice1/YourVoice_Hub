...
const AuthPage = () => {
  ...
  // --- Submit Handlers with Role-based Redirect ---
  const handleLogin = async (e) => {
    ...
    if (result.success) {
      ...
      if (result.user.role === 'ngo') navigate('/ngo');
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md bg-white border-2 border-teal-400 rounded-2xl shadow-xl p-8">
        ...
        <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
          <button
            onClick={() => { setActiveTab('login'); setError(''); setValidationErrors({}); }}
            className={`w-1/2 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
              activeTab === 'login' ? 'bg-white shadow text-gray-800' : 'text-gray-500'
            }`}
          >
            Login
          </button>
          <button
            onClick={() => { setActiveTab('register'); setError(''); setValidationErrors({}); }}
            className={`w-1/2 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
              activeTab === 'register' ? 'bg-white shadow text-gray-800' : 'text-gray-500'
            }`}
          >
            Register
          </button>
